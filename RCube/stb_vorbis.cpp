...
// get_info). *output will contain an array of float* buffers, one perstb_vorbis_open_file_section
...
extern int stb_vorbis_decode_filename(const char *filename, int *channels, unsigned int *sample_rate, short **output);
#endif
#if !defined(STB_VORBIS_NO_INTEGER_CONVERSION)
extern int stb_vorbis_decode_memory(const unsigned char *mem, int len, int *channels, unsigned int *sample_rate, short **output);
#endif
...
unsigned int stb_vorbis_get_file_offset(stb_vorbis *f)
{
   ...
   if (f->push_mode) return 0;
   #endif
   if (USE_MEMORY(f)) return (UINT)(f->stream - f->stream_start);
   #ifndef STB_VORBIS_NO_STDIO
...
#ifndef STB_VORBIS_NO_STDIO
int stb_vorbis_decode_filename(const char *filename, int *channels, unsigned int *sample_rate, short **output)
{
...
int stb_vorbis_decode_memory(const uint8 *mem, int len, int *channels, unsigned int *sample_rate, short **output)
{
   int data_len, offset, total, limit, error;
   short *data;
