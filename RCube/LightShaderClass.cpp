...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd , ID3D10Blob * Blob)
{
	bool result;
...
// The new light.vs and light.ps HLSL shader files are used as input to initialize the light shader.
	...
	result = InitializeShader(device, hwnd, L"Shaders/light.fx", L"Shaders/light.fx" , Blob);
	if(!result)
	{
		Shutdown();
		return false;
...
// The Render function now takes in cameraPosition, specularColor, and specularPower values and sends them into the SetShaderParameters function to make them active in the light shader before
// rendering occurs.
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX &worldMatrix, XMMATRIX &viewMatrix, 
			      ...
			      XMVECTOR &diffuseColor, XMFLOAT3 &cameraPosition, XMVECTOR &specularColor, float &specularPower , ID3D10Blob * Blob)
{
	bool result;
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename, ID3D10Blob * Blob)
{
	HRESULT result;
...
// One of the major changes to the shader initialization is here in the polygonLayout. We add a third element for the normal vector that will be used for lighting. The semantic name is NORMAL 
// and the format is the regular DXGI_FORMAT_R32G32B32_FLOAT which handles 3 floats for the x, y, and z of the normal vector. The layout will now match the expected input to the HLSL vertex shader.
...
// Here we setup the light constant buffer description which will handle the diffuse light color and light direction. Pay attention to the size of the constant buffers, if they are not multiples 
// of 16 you need to pad extra space on to the end of them or the CreateBuffer function will fail. In this case the constant buffer is 28 bytes with 4 bytes padding to make it 32.
...
bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX &worldMatrix, XMMATRIX &viewMatrix, 
					   XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT3 &lightDirection, 
					   XMVECTOR &ambientColor, XMVECTOR &diffuseColor, XMFLOAT3 &cameraPosition, XMVECTOR &specularColor, 
					   float &specularPower)
{
	HRESULT result;
...
//	viewMatrix*= XMMatrixScaling(0.1f,0.1f,0.1f);
	projectionMatrix = XMMatrixTranspose(projectionMatrix);
//	projectionMatrix *= XMMatrixScaling(0.1f,0.1f,0.1f);
...
// The light constant buffer is setup the same way as the matrix constant buffer. We first lock the buffer and get a pointer to it. After that we set the diffuse color and light direction 
// using that pointer. Once the data is set we unlock the buffer and then set it in the pixel shader. Note that we use the PSSetConstantBuffers function instead of VSSetConstantBuffers since 
// this is a pixel shader buffer we are setting.
