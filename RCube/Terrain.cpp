...
HRESULT Terrain::Initialize(HWND hwnd, D3DGlobalContext *D3DGC, 
...
bool Terrain::InitializeBuffers(ID3D11Device* device, int ClustersWidth, int ClusterHeight)
{
...
bool Terrain::GenersteRandTerrain( KFLandscapeParam* LandParam, float VertixesIndent) {
...
bool  Terrain::ReadTerrainFromTexture(KFLandscapeParam* LandParam, float VertixesIndent) {
...
void Terrain::GenerateLandScape(KFLandscapeParam* LandParam, HeightMapType* m_heightMap) {
...
int Terrain::GenerateRandRadius(int MaxRadius, int MinRadius) {
...
Terrain::Point Terrain::GenerateRandPoint() {
	Point Result;
...
int Terrain::ComeputeIndex(int X , int Z) {
...
void Terrain::Frame(bool CameraReplaseData, XMVECTOR &CameraPosition) {
...
void Terrain::CheckDrawLenghtParam(HWND hwnd) {

	if (g_DrawLength < 1) {

		MessageBox(hwnd, L"дальность прорисовки меньше минимально возможной устанавливаю значение 1.Terrain", L"Error", MB_OK);
		g_DrawLength = 1;
		return;
	...
	if ((g_DrawLength > (MinimalClusteringSize / 2) - 1) && g_DrawLength != 1) {
		...
		MessageBox(hwnd, L"дальность прорисовки больше максимально возможной устанавливаю максимальное значение.Terrain", L"Error", MB_OK);
...
void Terrain::CheckClustersVertixes(HWND hwnd) {

	bool Check;

	if (g_Rows % g_ClusterWidth != 0.0f) {
		MessageBox(hwnd, L"число вершин некратно числу кластеров по оси X, произвожу автоматическое приведение.Terrain", L"Error", MB_OK);
		Check = false;
	...
	if (g_Columns % g_ClusterHeight != 0.0f) {
		MessageBox(hwnd, L"число вершин некратно числу кластеров по оси Y, произвожу автоматическое приведение.Terrain", L"Error", MB_OK);
		Check = false;
...
void Terrain::LandParamChecker(KFLandscapeParam* LandParam) {

	if (LandParam->MinRadius < 1)
	{

		MessageBox(g_hwnd, L"Мининальный радиус высот/низин меньше 1, привожу к 1.Terrain", L"Error", MB_OK);
	...
	if (LandParam->MaxRadius <= LandParam->MinRadius)
	{
		...
		MessageBox(g_hwnd, L"максимальный радиус гор должен дыть больше чем минимальный.Привожу к коректным значениям.Terrain", L"Error", MB_OK);
	...
	if (LandParam->HeightesCout < 0) {
		...
		MessageBox(g_hwnd, L"количество высот не может быть меньше 0. привожу к 0.Terrain", L"Error", MB_OK);
	...
	if (LandParam->LowlandsCout < 0) {
		...
		MessageBox(g_hwnd, L"количество низин не может быть меньше 0. привожу к 0.Terrain", L"Error", MB_OK);
	...
	if (LandParam->MaxHeight < 0) {
		...
		MessageBox(g_hwnd, L"максимальное значение высоты гор не может быть меньше 0. привожу к 0.Terrain", L"Error", MB_OK);
	...
	if (LandParam->MinHeight < 0) {
		...
		MessageBox(g_hwnd, L"минимальное значение размера низин не может быть меньше 0. привожу к 0.Terrain", L"Error", MB_OK);
