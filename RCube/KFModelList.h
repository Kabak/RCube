...
public:
	...
	void Frame();
	std::vector <KFModel*> RCubeModelListNoClustring; // не используют кластеринг
	std::vector <KFModel*> RCubeModelListClustring; // используют кластеринг
	...
	KFModel::PositionType ** ObjPoses;// это массив который хранит данные об указателях на позиции и поворота для наиболее скоростного доступа без посредников

private:
 
	FrustumClass * gActiveFrustum;
	std::vector<bool> IsObjUseClustering; //массив описывающий использует ли тип модели кластеринг true использует false не использует формат заполнения [индекс типа модели]
	std::vector <int> PreviosFrameIndexes;// эта фигня нужна для оптимизации проверять предыдущий кадр
	...
	int * CoutObjetcs;
	std::vector<int> ActiveShaderIndexes;// массив котопый рписывает все индесса октивных шейдеров
	int ** ShaderObjArr;
