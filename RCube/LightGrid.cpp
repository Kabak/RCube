...
{
    // nothing
	lightIndexLists.resize( 1024 );
	coverageLists.resize( 1024 );
	tempBuffer.resize( 33816576 );
}
...
inline size_t LightGridBuilder::cellCount()
{
...
{
    for (size_t cellIndex = 0, i = cellCount(); cellIndex < i; ++cellIndex)
    {
//        lightIndexLists[cellIndex].resize(0);
//        coverageLists[cellIndex].resize(0);
		lightIndexLists[cellIndex].clear();
		coverageLists[cellIndex].clear();
//		lightIndexLists[cellIndex].erase( lightIndexLists[cellIndex].begin(), lightIndexLists[cellIndex].end() );
//		coverageLists[cellIndex].erase( coverageLists[cellIndex].begin(), coverageLists[cellIndex].end() );
    }
}
...
void LightGridBuilder::pushFragment(int cellIndex, int lightIndex, uint64_t coverage)
{
    ...
    lightIndexLists[cellIndex].emplace_back (lightIndex);
    coverageLists[cellIndex].emplace_back (coverage);
}

void LightGridBuilder::buildAndUpload(void* gpuBuffer/*, size_t bufferSize*/)
{
//    tempBuffer.resize(bufferSize + 262144);//256 * 1024); // 256 KB margin: max allocation per cell
//	tempBuffer.resize( 33816576 );//256 * 1024); // 256 KB margin: max allocation per cell

    size_t headerBytes = cellCount() * 1024;//64 * 16; // uint4: 16 bytes per entry
...
inline uint32_t swap_word_pair(uint32_t pair)
{
    return (pair << 16) | (pair >> 16);
}

inline void LightGridBuilder::buildFlatEntries(int& _x, int& _y, int& _z)
{
    ...
    for (int entryIndex = 0; entryIndex < 64; ++entryIndex)
    {
		...
		size_t allocBTemp = allocatedBytes / 16;
        assert( allocBTemp < 0x1000000);
        entry_ptr[0] = uint32_t(((allocBTemp) << 8) | list_size);

        allocatedBytes += (std::max(size_t(6), list_size) - 6 + 7) / 8 * 16;
//		allocatedBytes += (std::max(size_t(6), list_size) - 13) / 128;
	}
}
